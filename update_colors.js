...
// Replace the old variables completely
const startIdx = content.indexOf(':root[data-theme="dark"] {');
const endIdx = content.indexOf('body{');
if (startIdx !== -1 && endIdx !== -1) {
    content = content.substring(0, startIdx) + newRoot + '\n' + content.substring(endIdx);
}

// Remove old extra :root
content = content.replace(/:root\{\s*\}/g, '');

fs.writeFileSync(path, content, 'utf8');
console.log('app.scss variables updated.');
