...
class PluginManagementController extends BaseController
{
    ...
    public function upload(Request $request): BaseHttpResponse
    {
        ...
        $tempPath = storage_path('app/plugins/' . \Illuminate\Support\Str::random(10));

        try {
            $zipper = new \Botble\Base\Supports\Zipper();

            if (! $zipper->extract($file->getRealPath(), $tempPath)) {
                return $this
                    ->httpResponse()
                    ->setError()
                    ->setMessage(trans('packages/plugin-management::plugin.unzip_failed'));
            }
            ...
            if (! $pluginJsonFile) {
                ...
                return $this
                    ->httpResponse()
                    ->setError()
                    ->setMessage(trans('packages/plugin-management::plugin.missing_json_file'));
            }

            $manifest = BaseHelper::getFileData($pluginJsonFile->getRealPath());
            $pluginName = $manifest['id'] ?? $manifest['name'] ?? null;
            ...
            // Flatten plugin name (replace slashes with dashes) to avoid nested folders in platform/plugins
            $pluginName = str_replace(['/', '\\'], '-', $pluginName);

            $folder = $pluginJsonFile->getPath();

            if (File::exists(plugin_path($pluginName))) {
                File::deleteDirectory(plugin_path($pluginName));
            }

            File::copyDirectory($folder, plugin_path($pluginName));

            File::deleteDirectory($tempPath);

            return $this
                ->httpResponse()
                ->setMessage(trans('packages/plugin-management::plugin.install_success'));
        } catch (Exception $exception) {
            return $this
                ->httpResponse()
                ->setError()
