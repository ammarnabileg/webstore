...
class SpecificationAttributeForm extends FormAbstract
{
    public function setup(): void
    {
        ...
        $model = $this->getModel();
        $modelId = is_object($model) && method_exists($model, 'getKey') ? $model->getKey() : null;

        $defaultOptions = [];

        if ($modelId) {
            $defaultOptions = $model->getDefaultLanguageOptions();
        }

        $options = $defaultOptions;
        $translatedOptions = [];

        if ($isNotDefaultLanguage && $modelId) {
            $refLang = $this->request->input('ref_lang');
            $trans = DB::table('ec_specification_attributes_translations')
                ->where('ec_specification_attributes_id', $modelId)
            ...
            ->addMetaBox(
                MetaBox::make('specification-attribute-options')
                    ...
                    ->attributes([
                        ...
                        'style' => sprintf(
                            'display: %s;',
                            in_array(old('type', is_object($this->getModel()) ? $this->getModel()->type : null), [SpecificationAttributeFieldType::SELECT, SpecificationAttributeFieldType::RADIO]) ? 'block' : 'none;'
                        ),
                    ])
